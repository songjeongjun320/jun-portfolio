        ...
        {Object.keys(sectionMapping).map((section) => (
          <button
            ...
            className={`group flex items-center justify-end py-2 px-4 transition-all duration-300 ${
              activeSection === sectionMapping[section] ? 'bg-white/20' : 'hover:bg-white/10'
            }`}
          ...
          >
            ...
            <span
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                activeSection === sectionMapping[section] ? 'bg-white scale-125' : 'bg-white/50 group-hover:bg-white/75'
              }`}
            />
          </button>
        ))}
